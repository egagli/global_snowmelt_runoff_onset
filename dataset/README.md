...
- **`global_snowmelt_runoff_onset.zarr.tar.refs.json`** — a [Kerchunk](https://fsspec.github.io/kerchunk/) reference set that lets `xarray`/`fsspec` lazily read the individual chunks inside the Zenodo-hosted `.zarr.tar` archive without downloading the whole file. See [`dataset_utils/test_open_zarr_lazy.ipynb`](../dataset_utils/README.md) for a benchmark of this access pattern against direct Azure Zarr access, and [`dataset_utils/split_dataset.ipynb`](../dataset_utils/README.md) for reference file creation.
