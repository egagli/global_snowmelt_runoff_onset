...
| `processing/results/<v>/s1_input_volume.csv` | `select_tiles_to_process.ipynb` (final section) | **Sect. 2.2.3** S1 input volume: unique VV scenes × sampled asset sizes (replaces ">100 TB") |
| `processing/results/<v>/bulk_processing_stats.csv` | `check_tile_status.ipynb` | **Sect. 2.2.3** fleet compute accounting from commit history: CPU-core-hours, wall time, TB read at the 80 m overview (`stats.dest_gb`), throughput, peak RSS, platform mix — per WY + totals |
