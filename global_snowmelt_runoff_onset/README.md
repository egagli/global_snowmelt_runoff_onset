...
| [`processing.py`](processing.py) | The core algorithm: `get_sentinel1_rtc()` (Sentinel-1 RTC acquisition from Planetary Computer), `get_spatiotemporal_snow_cover_mask()` (MODIS-derived snow phenology masking — currently reads the old `MODIS_seasonal_snow_mask`-derived store; see root README migration notes), `apply_all_masks()`, `filter_insufficient_pixels_per_orbit()`, `calculate_runoff_onset()`, `dataarrays_to_dataset()`, and supporting helpers (temporal resolution, DOWY conversion, gap analysis). |
