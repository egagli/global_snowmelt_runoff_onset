...
Any of the batch workflows can be restricted to an explicit tile list via the `tiles_file` input — a repo-relative path with one `row,col` per line (inline `#` comments allowed). Two lists are maintained: `processing/tile_data/station_tiles_v10.txt` (built by `processing/scripts/make_station_tile_list.py`) for prioritized weather-station tiles, and `processing/tile_data/test_tiles_v10.txt`, the eight-tile validation battery to rerun after an algorithm change, dependency bump, or new water year. The battery replaces the throwaway `test_tiles.yml` shakedown workflow (deleted 2026-08-13 once the fleet completed) — dispatching it through `process_batch.yml` exercises the same concurrent-commit `ConflictDetector` path the old workflow did.
