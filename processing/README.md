...
| [`3_quality_check_tiles.ipynb`](3_quality_check_tiles.ipynb) | **Correctness gate on the data**, where `2_check_tile_status.ipynb` gates the ledger. Five ordered gates ending in one PASS/FAIL table, all logic in `global_snowmelt_runoff_onset/qc.py`: **schema** (dtype, `_FillValue` at both attr and zarr `fill_value`, scale factors, shard/chunk geometry, grid, coords — metadata only), **ledger vs data** (the commit `stats.valid_px` must *equal* the slab's valid-pixel count; an `empty` marker or absent commit must find an unwritten slab — the only check that catches a durably-wrong empty marker, i.e. the hemisphere trap), **invariants** (DOWY range, MAD ≥ 0, tr > 0, `runoff_onset` valid ⇒ `temporal_resolution` valid), **composites** (recomputed from the tile's own annual layers by the same `processing.median_*_with_min_obs`, required to agree within the int16 quantization — half a step for the composite's own rounding plus half a step for the source's, since the pipeline reduced the annuals in float32 while this recompute starts from the stored values; the second term only affects `temporal_resolution_median`, the one composite whose source is scaled — also detects a stale composite directly, which is off by days), and **tile seams** (median \|Δ\| across each tile boundary vs one pixel inside; nothing else in the pipeline looks for edge artifacts). Then a **regression fingerprint** of the battery (per tile-year valid px, DOWY percentiles, median tr, hash of the raw bytes) diffed against `results/<version>/qc_test_tile_fingerprint.csv` — this is what replaced "diff against v9" as the regression mechanism, since v9 is expected to differ and so can never fail — and a **visual pass** (`qc.view_tile`: composites over a UTM basemap, per-year onset, anomaly, and temporal resolution, styled via `plot_utils.variable_kw`). **Samples deliberately**: the eight-tile battery read from `tile_data/test_tiles_v10.txt` plus a seeded random draw, never all ~4,400 tiles. Reads at the config's `release_tag`. Gate results → `results/<version>/qc_gates.csv`. The v9↔v10 comparison survives as an off-by-default appendix with its expected-differences rubric. |
...
- `test_tiles_v10.txt` — git-tracked `row,col` list of the **eight-tile validation battery**, each tile chosen for a failure mode the others don't reach: (10,0) antimeridian, (9,138) scene-densest memory stress, (25,39) Rainier v9 cross-check, (28,65) data + `no_valid_pixels` empty-year mix, (25,190) catalog backfill, (80,240) `no_seasonal_snow` + data years, (56,69)/(57,69) the equator-straddling Cayambe pair. Per-tile rationale and the v9 index for each are in the file's comments. This is the standing regression set to rerun after an algorithm change, dependency bump, new water year, or store migration — dispatch it via the workflows' `tiles_file` input (concurrent dispatch also exercises icechunk's `ConflictDetector`), then compare in `3_quality_check_tiles.ipynb`, which reads this file directly (`qc.test_tiles`) rather than carrying its own copy of the list. Replaces the throwaway `test_tiles.yml` workflow, deleted 2026-08-13 once the fleet completed.
