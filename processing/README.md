...
| [`check_tile_status.ipynb`](check_tile_status.ipynb) | Read-only fleet-progress dashboard, safe to run while a fleet run is writing: per-tile / per-water-year status tables, all commit metadata (stats + provenance flattened to columns), throughput plots with a naive ETA, interactive `explore()` maps, the dropped-scene (`missing_assets`) list, and the remaining-work list. Everything derives from the icechunk commit history via `status.py`. |
