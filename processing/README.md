...
| [`extend_store_water_years.py`](scripts/extend_store_water_years.py) | Zarr v3 resize + coordinate append for a new water year in one icechunk commit (`--through-wy`, `--branch`, `--dry-run`); see *Adding new water years* above. |
...
| [`migrate_registry_to_extended_grid.py`](scripts/migrate_registry_to_extended_grid.py) | One-shot v9→v10 registry row shift (+2) with placeholders for the new rows; superseded by the 2026-08-03 full rerun of `select_tiles_to_process.ipynb`, kept for provenance. |
